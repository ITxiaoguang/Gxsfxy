...
public class UserCommentListAdapter extends RecyclerView.Adapter<UserCommentListAdapter.ViewHolder> {
    ...
    public UserCommentListAdapter(MyApplication application, Activity activity, ArrayList<HashMap<String, String>> arrayList) {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        try {
            ...
            holder.timeTextView.setText(hashMap.get("comment_time"));
            jsonObject = new JSONObject(hashMap.get("content_info"));
            holder.contentTextView.setText("");
            holder.nameTextView.setText("");

            switch (hashMap.get("comment_table")) {
                case "news":
                    String news_type = jsonObject.getString("news_type");
                    if (news_type.equals("通知公告")) {
                        holder.contentTextView.setText("评论了通知：");
                    } else {
                        holder.contentTextView.setText("评论了新闻：");
                    }
                    holder.contentTextView.append(Html.fromHtml(hashMap.get("comment_content"), mApplication.mImageGetter, null));
                ...
                case "dynamic":
                    ...
                    if (dynamic_type.equals("circle")) {
                        holder.contentTextView.setText("评论了圈子：");
                    }
                    if (dynamic_type.equals("topic")) {
                        holder.contentTextView.setText("评论了话题：");
                    }
                    holder.contentTextView.append(Html.fromHtml(hashMap.get("comment_content"), mApplication.mImageGetter, null));
                    ...
                    break;
                case "activity":
                    holder.contentTextView.setText("评论了活动：");
                    holder.contentTextView.append(Html.fromHtml(hashMap.get("comment_content"), mApplication.mImageGetter, null));
                    ...
                    new AlertDialog.Builder(mActivity)
                            ...
                            .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    ...
                                    switch (hashMap.get("comment_table")) {
                                        case "news":
                                            ajaxParams = new UserAjaxParams(mApplication, "userComment", "cancelNews");
                                            ajaxParams.put("news_id", hashMap.get("comment_tid"));
                                            mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=userComment&a=cancelNews", ajaxParams, null);
                                            break;
                                        case "dynamic":
                                            ajaxParams = new UserAjaxParams(mApplication, "userComment", "cancelDynamic");
                                            ajaxParams.put("dynamic_id", hashMap.get("comment_tid"));
                                            mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=userComment&a=cancelDynamic", ajaxParams, null);
                                            break;
                                        case "activity":
                                            ajaxParams = new UserAjaxParams(mApplication, "userComment", "cancelActivity");
                                            ajaxParams.put("activity_id", hashMap.get("comment_tid"));
                                            mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=userComment&a=cancelActivity", ajaxParams, null);
                                            break;
                                    }
                                    mArrayList.remove(holder.getAdapterPosition());
                                    ToastUtil.show(mActivity, "取消评论成功");
    ...
    public ViewHolder onCreateViewHolder(ViewGroup group, int viewType) {
        View view = LayoutInflater.from(group.getContext()).inflate(R.layout.item_list_user_comment, group, false);
