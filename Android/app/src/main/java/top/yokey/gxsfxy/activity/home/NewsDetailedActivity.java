...
public class NewsDetailedActivity extends AppCompatActivity {
    ...
    private void initData() {
        ...
        praiseBoolean = false;
        news_id = mActivity.getIntent().getStringExtra("news_id");
        news_link = mActivity.getIntent().getStringExtra("news_link");
        news_from = mActivity.getIntent().getStringExtra("news_from");
        news_time = mActivity.getIntent().getStringExtra("news_time");
        news_image = mActivity.getIntent().getStringExtra("news_image");
        news_title = mActivity.getIntent().getStringExtra("news_title");
        titleTextView.setText(mActivity.getIntent().getStringExtra("title"));

        if (TextUtil.isEmpty(news_id) || TextUtil.isEmpty(news_link)) {
            ToastUtil.show(mActivity, "传入的数据有误!");
            returnActivity();
        }

        if (titleTextView.getText().toString().equals("校报内容")) {
    ...
    private void initEven() {
        ...
        shareImageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                mApplication.startActivityShare(mActivity, "分享", news_title, news_from, news_image, "[" + news_link + "]");
            }
        });
    ...
    private void getJson() {
        ...
        mApplication.newsFinalHttp.get(news_link, new AjaxCallBack<Object>() {
            @Override
            public void onSuccess(Object o) {
                ...
                try {
                    ...
                    content = content.replace("/UploadFiles", "http://www.gxtc.edu.cn/UploadFiles");
                    content = content.replace("style=", "other=");
                    String header = "<p style='font-size:18px;color:#115FB2'>" + news_title + "</p>";
                    header = header + "<p style='font-size:14px;color:#666666'>来源 : " + news_from + " | 发布时间 : " + TimeUtil.decode(news_time) + "</p>";
                    header = header + "<hr size='1px' color='#CCCCCC'>";
                    if (mApplication.saveFlowCheckBoxBoolean) {
                        content = content.replace("<img", "<other");
                    }
                    mWebView.loadDataWithBaseURL(null, TextUtil.encodeHtml(header + content), "text/html", "UTF-8", null);
                    updateClick();
                    isComment();
                    isPraise();
                } catch (Exception e) {
    ...
    private void getComment() {
        ...
        ajaxParams.put("table", "news");
        ajaxParams.put("tid", news_id);
    ...
    private void updateClick() {
        ...
        BaseAjaxParams ajaxParams = new BaseAjaxParams(mApplication, "base", "newsClick");
        ajaxParams.put("news_id", news_id);
        mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=base&a=newsClick", ajaxParams, null);
    ...
    private void comment() {
        ...
        if (TextUtil.isEmpty(contentEditText.getText().toString())) {
            ...
            AndroidUtil.hideKeyboard(contentEditText);
            UserAjaxParams ajaxParams = new UserAjaxParams(mApplication, "userComment", "commentNews");
            ajaxParams.put("rid", rid);
            ajaxParams.put("news_id", news_id);
            ajaxParams.put("content", TextUtil.replaceFace(contentEditText.getText()));
            mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=userComment&a=commentNews", ajaxParams, new AjaxCallBack<Object>() {
                @Override
                public void onSuccess(Object o) {
                    super.onSuccess(o);
    ...
    private void praise() {
        ...
        UserAjaxParams ajaxParams = new UserAjaxParams(mApplication, "userPraise", "praiseNews");
        ajaxParams.put("news_id", news_id);
        mApplication.mFinalHttp.post(mApplication.apiUrlString + "c=userPraise&a=praiseNews", ajaxParams, null);
    ...
    private void cancelPraise() {
        ...
        UserAjaxParams ajaxParams = new UserAjaxParams(mApplication, "userPraise", "cancelNews");
        ajaxParams.put("news_id", news_id);
